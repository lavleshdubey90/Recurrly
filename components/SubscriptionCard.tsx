...
const SubscriptionCard = ({ name, price, billing, icon, currency, color, category, plan, renewalDate, onPress, expanded, paymentMethod, startDate, status }: SubscriptionCardProps) => {
    return (
            ...
            <View className='sub-head'>
                <View className='sub-main'>
                    ...
                    <View className="sub-copy">
                        ...
                        <Text numberOfLines={1} className='sub-meta' ellipsizeMode='tail'>{billing}</Text>
                    </View>
                </View>
                <View className='sub-price-box'>
                    <Text className='sub-price'>{formatCurrency(price, currency)}</Text>
                    <Text className='sub-billing'>{category?.trim() || plan?.trim() || (renewalDate ? formatSubscriptionDateTime(renewalDate) : '')}</Text>
                </View>
            </View>
